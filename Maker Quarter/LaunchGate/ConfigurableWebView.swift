...
struct ConfigurableWebView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        private func notifyBlocked() {
            DispatchQueue.main.async { self.onBlockedIfGate?() }
        }
        ...
        private func notifyReadyIfGate() {
            ...
            DispatchQueue.main.async { self.onGateShellReady?() }
        }

        private func notifyFailed() {
            DispatchQueue.main.async { self.onGateLoadFailed?() }
        }
        ...
        func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
            if blockGateEnabled { notifyFailed() }
        }
